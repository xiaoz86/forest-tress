...
export default function AboutPage() {
  ...
  return (
    <>
      ...
      {/* === Apple-style hero === */}
      <header className="relative pt-32 pb-16 px-6 text-center bg-gradient-to-b from-[#fafaf7] via-[#f5f5f0] to-[#faf8f2] max-md:pt-28 max-md:pb-12">
        ...
        <div className="max-w-[680px] mx-auto">
          <div className="inline-block text-[11px] tracking-[3px] text-moss uppercase mb-4 font-medium">
            ABOUT
          </div>
          <h1
            className="text-[clamp(2rem,4vw,2.8rem)] font-semibold tracking-[-0.02em] text-forest-deep leading-[1.2] max-md:text-[28px]"
      ...
      <section id="origin" className="py-24 px-10 bg-white max-md:py-16 max-md:px-5">
        ...
        <div className="max-w-[720px] mx-auto">
          {originSections['引言']?.map((text, i) => (
            <div key={`quote-${i}`} className="font-serif text-[clamp(1.2rem,2.6vw,1.6rem)] font-semibold text-forest-deep leading-[1.7] mb-8 pl-7 border-l-[3px] border-coral-soft">
              {text}
            </div>
          ))}
          ...
          {originSections['核心信念']?.map((text, i) => (
            <div key={`belief-${i}`} className="font-serif text-[1.05rem] font-semibold text-forest-deep leading-[1.8] py-6 px-7 bg-gradient-to-br from-love-pink/8 to-warmth/8 rounded-2xl border-l-[3px] border-coral-soft my-8"
      ...
      <section id="philosophy" className="py-24 px-10 bg-warm-cream max-md:py-16 max-md:px-5">
        ...
        <div className="grid grid-cols-[repeat(auto-fit,minmax(300px,1fr))] gap-7 max-w-[1100px] mx-auto">
          {philItems.map((item, i) => (
            <div key={i} className="group relative p-9 bg-white rounded-3xl border border-moss/8 transition-all duration-400 overflow-hidden hover:-translate-y-1 hover:shadow-[0_16px_48px_rgba(26,46,26,0.06)]">
      ...
      <section id="voices" className="py-24 px-10 bg-gradient-to-br from-forest-deep via-[#1f3a1f] to-forest-mid relative overflow-hidden max-md:py-16 max-md:px-5">
        ...
        <div className="grid grid-cols-[repeat(auto-fit,minmax(300px,1fr))] gap-6 max-w-[1100px] mx-auto relative z-[2]">
          {voiceItems.map((item, i) => {
            ...
            return (
              <div key={i} className="bg-white/[0.04] border border-white/[0.06] rounded-3xl py-8 px-7 backdrop-blur-[10px] transition-all hover:bg-white/[0.07] hover:border-white/10">
                <div className="flex items-center gap-3 mb-5">
                  ...
                  <div className="w-6 h-6 rounded-full bg-love-pink/20 flex items-center justify-center text-[0.65rem] ml-1">💛</div>
      ...
      <section className="py-24 px-10 bg-cream max-md:py-16 max-md:px-5">
        ...
        <div className="grid grid-cols-[repeat(auto-fit,minmax(240px,1fr))] gap-5 max-w-[1100px] mx-auto">
          {offlineItems.map((item, i) => (
            <div key={i} className="group bg-white rounded-2xl py-7 px-6 border border-moss/[0.06] transition-all hover:-translate-y-1 hover:shadow-[0_12px_36px_rgba(26,46,26,0.05)]">
              <div className="text-3xl mb-3">{item.emoji as string}</div>
      ...
      <section className="py-24 px-10 bg-gradient-to-b from-warm-cream to-white text-center max-md:py-16 max-md:px-5">
        ...
        <div className="max-w-[680px] mx-auto">
          ...
          <div className="inline-block px-7 py-3.5 bg-gradient-to-br from-love-pink/10 to-leaf/10 rounded-2xl font-serif text-[1rem] font-semibold text-forest-deep border border-love-pink/15 mt-4">
