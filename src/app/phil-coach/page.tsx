...
const FAQ_ITEMS: { question: string; answer: ReactNode }[] = [
  ...
  },
  {
    ...
    answer: (
      <>
        <p>
          对话内容不会写入附近森林的数据库；关掉这个标签页就散了，下次来它不会记得聊过什么。（为了不让你聊到一半去登录、回来发现内容没了，这段对话会临时存在你自己的浏览器里，关掉标签页即清除，不会上传给我们。）但诚实地说：模型服务商可能按它们的规则短期留存数据，所以我们不承诺全链路「零留存」。请不要写身份证号、住址、财务或医疗资料这类敏感信息，也请照顾好别人的隐私。聊到想留住的话，用对话下面的「把这段留给自己」带走它。
        </p>
        <p className="mt-3">
          另外说明白：走完第一条小径后继续使用，我们会请你留下称呼和微信号——这只是为了认识你、开通免费权限、邀你进社群，我们会记得<span className="text-white/70">你是谁与使用的时间</span>，但<span className="text-white/70">对话内容仍然不保存</span>（注册用户主动「留住」的除外）。
        </p>
        <p className="mt-3">
