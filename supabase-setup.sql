...
-- 在不在「遇见星空」（/sky）里。**创造者森林（/creators）不受它影响**——
-- 这是两件不同的事，所以要两个开关，而不是复用 status='hidden'：
-- 那个会把人从森林里一起摘掉，等于「想低调一点」只能用「整个消失」来表达。
--
-- 星空为什么需要单独一道闸：
--   一、**聚合**。星空把所有人的名字、城市、正在做的事放到同一屏。
--       每一条单看都已经是公开的，但聚合本身就改变了暴露程度。
--   二、**AI 推断会被公开**。「正在形成的星座」把成员的「优势与独特性」
--       「可以提供」「在寻找」喂给大模型，然后把模型的结论连人名一起发布
--       （「A 有场地、B 在找场地 → 可以一起做 X」）。
--       这和「列一张资料卡」是性质不同的一件事，理应能单独拒绝。
--
-- 默认 true：星空是这批人的另一种看法，不是新的信息披露；
-- 而且迁移前的老行没有这个值，判成 false 会让整片天空当场空掉。
alter table node_cards add column if not exists in_sky boolean not null default true;
